  ...
  <div class=" add-jms">
    ...
    <Group>
      ...
      <template v-if="agentType == 2">
        <Selector :options='areaList' title='代理城市' :value-map="['id','city']" direction='rtl' @on-change='changeArea' v-if="areaList"
          v-model='area'></Selector>
      </template>
      <template v-else-if='agentType == 3'>
        <Cell title='代理城市' v-model="cityAreaKey"></Cell>
        <Selector :options='qyList' title='代理区域' :value-map="['id','qy_name']" direction='rtl' @on-change='changeQy' v-if="areaList"
          v-model='qyAreaVal'></Selector>
      </template>
